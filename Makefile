...
fix-py:  ## fix python formatting with ruff
	python -m ruff check --fix spaday_trees
	python -m ruff format spaday_trees
...
test-py:  ## run python tests
	python -m pytest -v spaday_trees/tests

# alias
tests-py: test-py

coverage-py:  ## run python tests and collect test coverage
	python -m pytest -v spaday_trees/tests --cov=spaday_trees --cov-report term-missing --cov-report xml
...
pyodide-example: build  ## build the standalone Pyodide example into dist/lite
	test -n "$(firstword $(wildcard dist/spaday_trees-*.whl))"
...
test-pyodide-example: pyodide-example  ## run the standalone Pyodide example in Chromium
	...
	cd js; SPADAY_TREES_PYODIDE_ONLY=1 pnpm exec playwright test tests/pyodide.spec.js
